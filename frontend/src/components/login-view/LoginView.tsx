...
export const LoginView = ({
  ...
  const onCreateRoom = async () => {
    const roomId = await createRoom(currentUserId);
    await createGameSession(roomId);
    joinRoom(roomId);
  };
  ...
  return (
    <div className={styles.LoginView}>
      ...
      <div className={styles.LoginView__container}>
        ...
        {!isAuthenticated ? (
          <Button variant="contained" fullWidth size="large" onClick={() => loginWithRedirect()}>
            Войти
          </Button>
        ) : (
          <div className={styles.LoginView__buttons}>
            {currentRoomId ? (
              ...
              <Button variant="contained" fullWidth size="large" onClick={onCreateRoom}>
                Создать комнату
              </Button>
            )}
            <Snackbar
